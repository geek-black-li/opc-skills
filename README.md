...
文档精简重复表述，不省略必要决策。技术设计区分概要／详细设计与完整项目／局部修改：完整项目交代技术组成及适用的数据库、接口、前端实现；详细设计明确本期字段、约束、调用和错误处理，也可引用已核对的权威定义。概要通过后可先安排补充设计任务，不能直接视为全部可开发。PRD按功能补充输入输出、格式和计算规则，相关但未知的要求保留待确认，不当作“不适用”删除。
