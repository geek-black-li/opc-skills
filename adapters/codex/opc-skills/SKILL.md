...
2. 从真实路径向上查找 `skill-manifest.yaml`，将其目录作为 `OPCSKILLS_ROOT`。
3. 若未找到，再检查用户明确提供的仓库路径。仍未找到时停止，并要求用户提供 OPCSkills 本地路径。
...
$opc-skills 使用 zx-project-organizer，引导我创建一个全新项目
$opc-skills 使用 zx-project-organizer，引导我整理现有项目
$opc-skills 使用 zx-project-organizer，按 ZX 完整结构初始化当前项目
$opc-skills 使用 zx-project-organizer，按项目实际情况自适应初始化
$opc-skills 确认执行项目结构提案 zpo-0123456789abcdef
$opc-skills 放弃项目结构提案 zpo-0123456789abcdef
